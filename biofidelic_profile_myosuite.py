...
# =============================================================================
# 1B. MYOSUITE-INSPIRED MUSCLE CONDITION PROFILE
# =============================================================================

def _age_force_retention(age: float, sex: str) -> float:
    """Relative age-only force retention, normalized to the subject's young-adult peak.

    This intentionally excludes the baseline male/female strength offset because the
    chronic sex effect is already handled elsewhere in the model. The returned value
    therefore isolates age-linked sarcopenic weakening only.
    """
    age = float(age)
    sex = str(sex).lower()
    ...
    if age <= 60.0:
        ...
        return float(np.exp(-rate_per_yr * (age - 30.0)))

    torque_at_60 = np.exp(-(0.0080 if sex == 'male' else 0.0090) * 30.0)
    rate_post60 = 0.0150 if sex == 'male' else 0.0140
    return float(torque_at_60 * np.exp(-rate_post60 * (age - 60.0)))


def myosuite_muscle_condition_profile(age: float, sex: str, body_mass_kg: float = 70.0) -> dict:
    """Hybrid age/fatigue profile anchored to MyoSim / MyoSuite design choices.

    References implemented here:
      * MyoSim fatigue model uses a dynamic fatigue coefficient k_fatigue = 1.0.
      * MyoSim advanced sarcopenia is modeled as a global 50% reduction in maximal
        muscle force.

    We keep those anchors, then map them into the existing event-based fall pipeline:
      * age-specific sarcopenia severity is derived from the subject's age-only force
        retention curve and capped at the 50% MyoSim "advanced sarcopenia" ceiling;
      * the existing collapse model remains event-driven, but its floor, decay, and
        recovery are modulated by the age-linked sarcopenic severity.
    """
    age = float(age)
    sex = str(sex).lower()
    _ = float(max(body_mass_kg, 25.0))

    retention = _age_force_retention(age, sex)
    sarcopenia_force_loss = float(np.clip(1.0 - retention, 0.0, 0.50))
    severity = float(np.clip(sarcopenia_force_loss / 0.50, 0.0, 1.0))

    # MyoSim / MyoSuite fatigue anchor.
    fatigue_coefficient = 1.0

    # Map normalized phase progress to an approximate active-effort integral.
    # This scale is a simulator bridge parameter, not a literature constant.
    active_effort_scale = float(np.clip(1.80 + 0.35 * severity, 1.80, 2.15))

    min_factor = 0.07 - 0.045 * severity
    if sex == 'female' and age >= 65.0:
        min_factor -= 0.003 * min(1.0, (age - 65.0) / 20.0)
    min_factor = float(np.clip(min_factor, 0.02, 0.07))

    decay_time = int(round(45 - 15 * severity))
    decay_time = max(20, decay_time)
    recovery_time = int(round(120 + 40 * severity))

    return {
        'age_force_retention': retention,
        'sarcopenia_force_loss': sarcopenia_force_loss,
        'severity': severity,
        'fatigue_coefficient': float(fatigue_coefficient),
        'active_effort_scale': active_effort_scale,
        'min_factor': min_factor,
        'decay_time': decay_time,
        'recovery_time': recovery_time,
        'reference_advanced_sarcopenia_force_loss': 0.50,
        'reference_fatigue_coefficient': 1.0,
        'source': 'myosim_muscle_fatigue_and_sarcopenia_bridge',
    }
...
def weakening_config(age: float, sex: str, body_mass_kg: float = 70.0) -> dict:
    ...
    cond = myosuite_muscle_condition_profile(age, sex, body_mass_kg)
    return {
        'initial_factor': 1.0,
        'min_factor': float(cond['min_factor']),
        'decay_time': int(cond['decay_time']),
        'recovery_time': int(cond['recovery_time']),
        'fatigue_coefficient': float(cond['fatigue_coefficient']),
        'active_effort_scale': float(cond['active_effort_scale']),
        'sarcopenia_force_loss': float(cond['sarcopenia_force_loss']),
        'sarcopenia_severity': float(cond['severity']),
        'source': cond['source'],
    }
...
def get_age_style_v2(age_years: float, height_m: float = 1.75, sex: str = 'male', body_mass_kg: float = 70.0) -> dict:
    ...
    return {
        ...
        'myosuite_sarcopenia_force_loss': float(myosuite_muscle_condition_profile(age, sex, body_mass_kg)['sarcopenia_force_loss']),
        'myosuite_fatigue_coefficient': float(myosuite_muscle_condition_profile(age, sex, body_mass_kg)['fatigue_coefficient']),
    }
...
def print_subject_profile(age: float, sex: str, height_m: float, body_mass_kg: float, verbose: bool = True) -> dict:
    ...
    muscle_cond = myosuite_muscle_condition_profile(age, sex, body_mass_kg)
    ...
    if verbose:
        ...
        print(f"    stoop stand/walk              : {style['stand_stoop_target_deg']:.1f} deg / {style['walk_stoop_target_deg']:.1f} deg")
        ...
        print('  MYOSUITE-INSPIRED MUSCLE CONDITIONING')
        print(f"    sarcopenia force loss         : {muscle_cond['sarcopenia_force_loss']:.1%}")
        print(f"    fatigue coefficient (k)       : {muscle_cond['fatigue_coefficient']:.2f}")
        print(f"    effort integral bridge scale  : {muscle_cond['active_effort_scale']:.2f}")
