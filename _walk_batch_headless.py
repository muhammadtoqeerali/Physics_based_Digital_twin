...
v8 adds two non-invasive improvements - no effect on controller stability:
  ...
  IMU  - Physics pipeline: anti-aliasing pre-filter ? noise+bias ? soft-tissue
         artifact ? hardware saturation (±16g accel, ±2000°/s gyro) ?
...
import sys, os
import re
sys.path.insert(0, '/mnt/hdd16T/ToqeerHomeBackup/mujoco_project')
import numpy as np
import torch
import mujoco
...
def _geom_vertical_half_extent(mj_model, mj_data, geom_id):
    """Approximate world-space vertical half-extent for floor re-grounding."""
    g = int(geom_id)
    gtype = int(mj_model.geom_type[g])
    size = np.asarray(mj_model.geom_size[g], dtype=float)
    xmat = np.asarray(mj_data.geom_xmat[g], dtype=float).reshape(3, 3)
    local_z = xmat[2, :]
    if hasattr(mujoco, 'mjtGeom'):
        mgeom = mujoco.mjtGeom
        if gtype == int(mgeom.mjGEOM_SPHERE):
            return float(size[0])
        if gtype in (int(mgeom.mjGEOM_CAPSULE), int(mgeom.mjGEOM_CYLINDER)):
            radial = float(size[0]) * float(np.linalg.norm(local_z[:2]))
            axial = float(size[1]) * abs(float(local_z[2])) if size.size > 1 else 0.0
            return float(radial + axial)
        if gtype == int(mgeom.mjGEOM_BOX):
            return float(np.sum(np.abs(local_z) * size[:3]))
        if gtype == int(mgeom.mjGEOM_ELLIPSOID):
            return float(np.sqrt(np.sum((local_z[:3] * size[:3]) ** 2)))
    rbound = getattr(mj_model, 'geom_rbound', None)
    if rbound is not None and len(rbound) > g:
        return float(max(0.0, rbound[g]))
    return float(max(size[0], np.max(size[:min(3, size.size)]))) if size.size else 0.0


def reground_character_to_floor(mj_model, mj_data, clearance=0.004):
    """Shift the free root vertically so the lowest geom sits just above the floor."""
    if getattr(mj_model, 'nq', 0) < 3:
        return 0.0
    mujoco.mj_forward(mj_model, mj_data)
    min_bottom = np.inf
    plane_type = getattr(getattr(mujoco, 'mjtGeom', object), 'mjGEOM_PLANE', None)
    for g in range(mj_model.ngeom):
        if plane_type is not None and int(mj_model.geom_type[g]) == int(plane_type):
            continue
        gname = (mujoco.mj_id2name(mj_model, MJOBJ_GEOM, g) or '').lower()
        if 'floor' in gname or 'ground' in gname or 'plane' in gname:
            continue
        bottom = float(mj_data.geom_xpos[g][2]) - _geom_vertical_half_extent(mj_model, mj_data, g)
        if bottom < min_bottom:
            min_bottom = bottom
    if not np.isfinite(min_bottom):
        return 0.0
    dz = float(clearance - min_bottom)
    if abs(dz) > 1e-6:
        mj_data.qpos[2] += dz
        if getattr(mj_data, 'qvel', None) is not None and mj_data.qvel.shape[0] >= 3:
            mj_data.qvel[2] = 0.0
        mujoco.mj_forward(mj_model, mj_data)
    return dz


def _apply_subject_anthropometry_to_env(env, age, height, weight, sex):
    mj_model = env.unwrapped.model
    AnthropometricModel(mj_model, age=age, height=height, weight=weight, sex=sex)
    return mj_model


def _reset_subject_scaled_env(env, clearance=0.004):
    obs, info = env.reset()
    reground_character_to_floor(env.unwrapped.model, env.unwrapped.data, clearance=clearance)
    return obs, info


def estimate_subject_vulnerability(age_style):
    age_years = float(age_style.get('age_years', age_style.get('age', 35.0)))
    height_m = float(age_style.get('height_m', 1.70))
    strength = float(age_style.get('muscle_strength_factor', age_style.get('strength_factor_raw', 1.0)))
    balance = float(age_style.get('balance_impairment', 0.0))
    ds = float(age_style.get('expected_double_support', 0.25))
    age_term = np.clip((age_years - 55.0) / 25.0, 0.0, 1.0)
    short_term = np.clip((1.58 - height_m) / 0.22, 0.0, 1.0)
    weakness = np.clip(1.0 - strength, 0.0, 1.0)
    ds_term = np.clip((ds - 0.24) / 0.18, 0.0, 1.0)
    vuln = 0.36 * balance + 0.28 * weakness + 0.20 * age_term + 0.10 * short_term + 0.06 * ds_term
    return float(np.clip(vuln, 0.0, 1.0))
...
print("=" * 70)
print("  Realistic Backward Fall Simulation - Biofidelic Implementation v25-armquietsettle")
...
PHASES = phase_timing(SIM_AGE, SIM_SEX)
TOTAL_STEPS = sum(PHASES.values())
...
def compute_subject_control_targets(age_style, natural_speed, natural_ds):
    """Use a speed-acceptance band so the controller does not fight the latent gait."""
    ...
    expected_ds = float(age_style.get('expected_double_support', 0.30))
    label = str(age_style.get('label', '35_to_55'))
    ...
    band = float(acceptance_bands.get(label, 0.32))
    speed_gap = abs(natural_speed - target_speed) / max(target_speed, 1e-6)

    if speed_gap <= band:
        control_speed = natural_speed
        speed_source = 'natural_accepted'
    else:
        mix = 0.55 if label in ('under_35', '35_to_55') else 0.35
        control_speed = (1.0 - mix) * natural_speed + mix * target_speed
        speed_source = 'blended'

    control_speed *= control_gain
    control_speed = float(np.clip(
        control_speed,
        float(age_style.get('min_adapted_speed', 0.18)),
        float(age_style.get('max_speed_ratio', 1.8)) * natural_speed,
    ))
    control_ds = float(np.clip(0.25 * natural_ds + 0.75 * expected_ds, 0.18, 0.52))

    print(
        f"      [SpeedAccept] natural={natural_speed:.3f}  target={target_speed:.3f}  "
        f"gap={speed_gap:.1%}  band={band:.0%}  control={control_speed:.3f}  source={speed_source}"
    )
    return {
...
def infer_z_walk_stable(n_samples=1000, age=None, z_stand=None):
    global LAST_Z_WALK_DIAGNOSTICS
    """
    Infer robust walking embedding using weighted regression and then evaluate
    candidates on the *actual* anthropometrically modified subject body.
    """
    age = SIM_AGE if age is None else age
    age_style = get_age_style(age, SIM_HEIGHT, SIM_SEX, body_mass_kg=SIM_RESOLVED_WEIGHT)
    age_style = dict(age_style)
    age_style['vulnerability_index'] = estimate_subject_vulnerability(age_style)
    latent_style = get_age_style(35, height_m=1.70, sex='male', body_mass_kg=70.0)
    print(
        f"  Inferring z_walk (stable locomotion | reference_straight_walk_speed={latent_style['target_walk_speed']:.2f} m/s "
        f"| subject_age_style={age_style['label']} | subject_target_speed={age_style['target_walk_speed']:.2f} m/s "
        f"| vulnerability={age_style['vulnerability_index']:.2f})..."
    )
    ...
    _apply_subject_anthropometry_to_env(env_tmp, SIM_AGE, SIM_HEIGHT, SIM_RESOLVED_WEIGHT, SIM_SEX)
    ...
    for trial in range(80):
        ...
        z = model.sample_z(1)
        obs, _ = _reset_subject_scaled_env(env_tmp)
    ...
    if len(observations) < 50:
        ...
        _apply_subject_anthropometry_to_env(env_fb, SIM_AGE, SIM_HEIGHT, SIM_RESOLVED_WEIGHT, SIM_SEX)
        for trial in range(30):
            torch.manual_seed(SEED + trial + 9000)
            z = model.sample_z(1)
            obs_fb, _ = _reset_subject_scaled_env(env_fb)
            trial_rewards = []
            for _ in range(60):
        ...
        LAST_Z_WALK_DIAGNOSTICS = {
            'mean_reward': float(best_rew),
            'mean_vx': 0.0,
            'straightness': 0.0,
            'double_support_frac': 0.5,
            'gait_score': float(best_rew),
            'age_target_speed': float(age_style['target_walk_speed']),
            'vulnerability_index': float(age_style['vulnerability_index']),
        }
        return best_z
    ...
    print("      Selecting best z_walk via rollout evaluation (subject-scaled multi-objective gait search)...")
    best_z = None
    best_score = -np.inf
    best_reward = -np.inf
    scored_candidates = []
    eval_env, _ = make_humenv(task="move-ego-0-2")
    _apply_subject_anthropometry_to_env(eval_env, SIM_AGE, SIM_HEIGHT, SIM_RESOLVED_WEIGHT, SIM_SEX)
    reground_character_to_floor(eval_env.unwrapped.model, eval_env.unwrapped.data, clearance=0.004)
    ...
    def _support_flags(model, data, left_id, right_id):
        ...
        for ci in range(data.ncon):
            ...
            names = (
                (mujoco.mj_id2name(model, MJOBJ_GEOM, g1) or '').lower(),
                (mujoco.mj_id2name(model, MJOBJ_GEOM, g2) or '').lower(),
            )
            is_ground = any(('floor' in n or 'ground' in n or 'plane' in n) for n in names) or any(
                model.geom_type[g] == plane_type for g in (g1, g2)
            )
            if not is_ground:
                continue
    ...
    def _evaluate_candidate(z_cand, cand_label, cand_index, total_count):
        obs_e, _ = _reset_subject_scaled_env(eval_env)
        rollout_rewards = []
        xs, ys, vxy = [], [], []
        dbl = []
        step_transitions = []
        prev_contact_state = (False, False)
        for _ in range(75):
            ...
            if foot_left >= 0 and foot_right >= 0:
                ...
                curr = (bool(lsup), bool(rsup))
                if curr != prev_contact_state:
                    step_transitions.append(len(dbl))
                prev_contact_state = curr
            if t or tr:
                break
        ...
        if len(xs) > 1:
            ...
            lateral_offsets = [
                float(np.dot(np.array([x - xs[0], y - ys[0]], dtype=float), lat_axis))
        ...
        if len(step_transitions) >= 3:
            target_cadence_hz = float(age_style['target_walk_speed']) / max(0.530 * float(age_style.get('height_m', 1.70)), 0.3)
            intervals = np.diff(step_transitions)
            mean_interval_steps = float(np.mean(intervals)) if len(intervals) else 0.0
            natural_interval = 30.0 / max(2.0 * target_cadence_hz, 0.5)
            cadence_score = float(np.clip(1.0 - abs(mean_interval_steps - natural_interval) / max(natural_interval, 1e-6), 0.0, 1.0))
        else:
            cadence_score = 0.0
        gait_score = (
            0.26 * mean_r +
        ...
        print(
            f"        z_candidate {cand_index}/{total_count} ({cand_label})  reward={mean_r:.3f}  vxy={mean_speed:.3f}  "
            f"straight={straightness:.3f}  lat_rms={lateral_rms:.3f}  ds={ds_frac:.2f}  cadence={cadence_score:.2f}  score={gait_score:.3f}"
        )
        return gait_score, mean_r, diag

    total_candidates = len(z_candidates)
    for idx, (cand_label, z_cand) in enumerate(z_candidates, start=1):
        gait_score, mean_r, diag = _evaluate_candidate(z_cand, cand_label, idx, total_candidates)
        scored_candidates.append((gait_score, z_cand.clone(), diag))
        if gait_score > best_score:
            best_score = gait_score
            best_reward = mean_r
            best_z = z_cand.clone()
            LAST_Z_WALK_DIAGNOSTICS = dict(diag)

    vuln = float(age_style['vulnerability_index'])
    if vuln > 0.30 and scored_candidates:
        print("      [Z-Refine] Vulnerable subject - running local z refinement...")
        top3 = sorted(scored_candidates, key=lambda item: -item[0])[:3]
        for refine_round in range(12):
            base_score, base_z, base_diag = top3[refine_round % len(top3)]
            torch.manual_seed(SEED + 8000 + refine_round)
            noise = torch.randn_like(base_z) * 0.035
            z_refined = base_z + noise
            gait_score, mean_r, diag = _evaluate_candidate(z_refined, f"refine={refine_round + 1}", refine_round + 1, 12)
            scored_candidates.append((gait_score, z_refined.clone(), diag))
            if gait_score > best_score:
                best_score = gait_score
                best_reward = mean_r
                best_z = z_refined.clone()
                LAST_Z_WALK_DIAGNOSTICS = dict(diag)

    blend_with_stand = False
    stand_blend = 0.0
    best_diag = dict(LAST_Z_WALK_DIAGNOSTICS) if LAST_Z_WALK_DIAGNOSTICS else {}
    if age_style['label'] == '70_plus' and best_diag.get('mean_vx', 1.0) > 1.3 * age_style['target_walk_speed']:
        blend_with_stand = True
        stand_blend = 0.20
    elif vuln > 0.50:
        blend_with_stand = True
        stand_blend = 0.15

    if blend_with_stand and z_stand is not None and best_z is not None:
        best_z = (1.0 - stand_blend) * best_z + stand_blend * z_stand
        LAST_Z_WALK_DIAGNOSTICS['stand_blend'] = float(stand_blend)
        print(f"      [Z-Blend] Blended z_walk with z_stand ({stand_blend:.0%} stand) for age/vulnerability")

    eval_env.close()
    print(
        f"      Best z_walk selected (score={best_score:.3f} | reward={best_reward:.3f} | "
        f"vxy={LAST_Z_WALK_DIAGNOSTICS.get('mean_vx', 0.0):.3f})"
    )
...
class BiofidelicFallController:
    ...
    def _configure_contact_solver(self):
        ...
        if hasattr(opt, 'impratio'):
            opt.impratio = max(float(opt.impratio), 10.0)
        if hasattr(opt, 'tolerance'):
            opt.tolerance = min(float(opt.tolerance), 1e-8)
    ...
    def apply_impact_brake(self):
        if not self.impact_brake_mode or self.mj_data.qvel.shape[0] < 6:
            return
        # Grounded anti-slip damper: do not spike contact forces with global
        # friction/damping changes; just kill root tangential drift once the body
        # has actually touched down.
    ...
    def activate_rest_mode(self, z_rest=None):
        ...
        # increase passive damping to kill chatter
        self.mj_model.dof_damping[:] = self.original_dof_damping * 2.4
        # increase contact friction to reduce floor slide
        for g in range(self.mj_model.ngeom):
        ...
        # give just enough torque to finish lying down, not enough to chatter
    ...
    def update_muscle_weakening(self, phase_progress, weakening_type='exponential'):
        if weakening_type == 'exponential':
            self.leg_strength = WEAKENING_CONFIG['initial_factor'] * \
                                np.exp(-3 * phase_progress)
            self.leg_strength = max(self.leg_strength, WEAKENING_CONFIG['min_factor'])
        else:
            self.leg_strength = WEAKENING_CONFIG['initial_factor'] - \
                                phase_progress * (WEAKENING_CONFIG['initial_factor'] -
                                                  WEAKENING_CONFIG['min_factor'])
    ...
    def get_action(self, obs, z):
        ...
        if self.rest_mode:
            ...
            if pelvis_low and grounded:
                # Once the body is already down on the floor, keep the arms from
                # chattering. This preserves the protective reach during descent,
                # then aggressively quiets the wrists/elbows after impact.
                action[self.arm_actuators] *= 0.02
                action[self.torso_actuators] *= 0.18
                action[self.leg_actuators] *= 0.30
...
class IMUValidator:
    ...
    def __init__(self, mj_model, mj_data, sensor_body='Torso', age=35, height=1.75,
                 target_output_hz=100.0, mount_label=None, sensor_offset_local=None):
        self.mj_model = mj_model
        self.mj_data  = mj_data
        self.age      = age
        self.height   = height
        ...
        print(f"      [IMU] body={self.body_name} (id={self.body_id})  opt_dt={opt_dt:.5f}s  nsubsteps~{ns}  "
    ...
    def get_sensor_mount_info(self):
        ...
        return {
            ...
            'mount_type': f'{self.mount_label} proxy',
    ...
    def get_runtime_processing_report(self):
        return {
            ...
            'actual_point_kinematics': 'rigid-body point correction v = v0 + ?×r, a = a0 + a×r + ?×(?×r)',
...
class SISFallValidator:
    ...
    Bagalà 2012: 25-130 m/s^2; 7-tap Hanning filtered, 30Hz sampling).
    """
    SISFALL_RANGES = {
    ...
    def validate_sisfall_signature(self, imu_data_buffer, fall_type,
        ...
        # Reference: Bagalà 2012 J NeuroEng; Casilari 2017 Sensors.
        if len(acc_mags) >= 7:
            k7         = np.hanning(7)
            k7         = k7 / k7.sum()
...
def run_enhanced_simulation(fall_type='backward_walking',
    ...
    reground_character_to_floor(mj_model_enh, mj_data_enh, clearance=0.004)
...
print(f"  [FallTypeLibrary]         scenario='{ACTIVE_FALL_TYPE}' | "
      f"force_dir={fall_config['force_direction']}")
...
print(f"  [GaitPhaseDetector]       ACTIVE - biofidelic perturbation timing")
...
from datetime import datetime
imu_filename = (f"fall_{ACTIVE_FALL_TYPE}_age{SIM_AGE}_"
                f"{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv")
try:
    result = imu.export_to_csv(imu_filename, metadata={
        'age': SIM_AGE, 'height': SIM_HEIGHT, 'sex': SIM_SEX,
        'weight': age_params.get('body_mass_kg', float(np.sum(mj_model.body_mass))), 'body_mass_kg': age_params.get('body_mass_kg', float(np.sum(mj_model.body_mass))), 'fall_type': ACTIVE_FALL_TYPE,
    })
...
try:
    marker_prefix = imu_filename.replace('.csv', '')
...
report_file = imu_filename.replace('.csv', '_validation.txt')
