...
from biofidelic_profile_myosuite import (
    get_age_style_v2,
...
_dyn_force = perturbation_force_config(MODEL_BODY_MASS_FOR_FORCE, SIM_AGE, SIM_SEX, bw_fraction=1.15)
...
print(f"""
  ...
  +---------------------------------------------------------------------+
  |  STAND      {PHASES['stand']:3d}      ~{PHASES['stand']/30:.1f}s       Stabilize standing posture          |
  |  WALK       {PHASES['walk']:3d}      ~{PHASES['walk']/30:.1f}s      Establish walking gait               |
  |  PERTURB    {PHASES['perturb']:3d}       ~{PHASES['perturb']/30:.1f}s        Gradual force application            |
  |  REACT      {PHASES['react']:3d}       ~{PHASES['react']/30:.1f}s        Human reaction time delay            |
  |  FALL       {PHASES['fall']:3d}      ~{PHASES['fall']/30:.1f}s      Collapse and settle                  |
  +---------------------------------------------------------------------+
  Controller : EnhancedBiofidelicController  (protective reflexes ON)
...
actual_step_marks = {'walk_start': None, 'perturb_start': None, 'react_start': None, 'fall_start': None}
...
with mujoco.viewer.launch_passive(mj_model, mj_data) as viewer:
    ...
    for step in range(TOTAL_STEPS):
        ...
        if step < phase_boundaries[1]:
            ...
            if actual_step_marks['walk_start'] is None:
                actual_step_marks['walk_start'] = int(phase_boundaries[1])
            ...
            if actual_step_marks['perturb_start'] is None:
                phase = 'walk'
                walk_diag = compute_preperturb_walk_metrics(controller, mj_model, mj_data, MODEL_BODY_MASS, age_params)
                gait_window_ok = controller.gait_detector.is_in_window(ACTIVE_FALL_TYPE)
                if walk_diag['ready']:
                    walk_stable_counter += 1
                    if walk_diag.get('ready_mode') == 'policy_limited':
                        policy_limited_counter += 1
                    else:
                        policy_limited_counter = 0
                else:
                    walk_stable_counter = 0
                    policy_limited_counter = 0
                nominal_walk_done = step >= phase_boundaries[2]
                force_start = step >= (phase_boundaries[2] + walk_extra_budget)
                fallback_force_start = step >= (phase_boundaries[2] + walk_extra_budget + 45)
                hard_force_start = step >= (phase_boundaries[2] + walk_extra_budget + 90)
                basic_walk_ok = (abs(walk_diag['speed']) >= 0.70 * controller.walk_target_speed and
                                 walk_diag['xcom_margin'] > -0.22 and
                                 walk_diag['grf_bw'] < 2.6)
                allow_perturb = (((walk_stable_counter >= 4) or (policy_limited_counter >= 6 and walk_diag.get('policy_limited', False))) and gait_window_ok) or                                  (force_start and gait_window_ok and basic_walk_ok) or                                  fallback_force_start or                                  hard_force_start
                if nominal_walk_done and allow_perturb:
                    actual_step_marks['perturb_start'] = int(step)
                    phase = 'perturb'
                    print(f"  [Step {step}] Perturbation started (gradual force) [FallTypeLibrary dir={fall_config['force_direction']}]")
                    if walk_diag.get('ready_mode') == 'policy_limited':
                        print("      gait gate note: policy-limited stable gait accepted (literature target not reachable with current walk prior)")
                    elif hard_force_start and not basic_walk_ok:
                        print("      gait gate fallback: hard timeout reached; perturbing with best available gait state")
                    elif fallback_force_start and not basic_walk_ok:
                        print("      gait gate fallback: waiting for next gait window, then perturbing")
                    print(f"      gait gate: ready={walk_diag['ready']} mode={walk_diag.get('ready_mode','none')} stable_steps={walk_stable_counter} gait_window={gait_window_ok} speed={walk_diag['speed']:.3f} Froude={walk_diag['froude']:.3f} XCoM={walk_diag['xcom_margin']:+.3f} ref_ratio={walk_diag.get('reference_speed_ratio', 0.0):.2f} DS={walk_diag['double_support']}")
                    if dynamics_frame is not None:
                        bw_now = max(MODEL_BODY_MASS * 9.81, 1.0)
                        lf_bw = float(dynamics_frame.get('left_foot_vertical_n_filt', dynamics_frame.get('left_foot_vertical_n', 0.0)) / bw_now)
                        rf_bw = float(dynamics_frame.get('right_foot_vertical_n_filt', dynamics_frame.get('right_foot_vertical_n', 0.0)) / bw_now)
                        sup_bw = float(dynamics_frame.get('support_vertical_n_filt', dynamics_frame.get('support_vertical_n', 0.0)) / bw_now)
                        tan_ratio = float(dynamics_frame.get('tangential_ratio_filt', dynamics_frame.get('tangential_ratio', 0.0)))
                        cop_body = dynamics_frame.get('support_cop_body_xy', None)
                        cop_str = 'n/a' if cop_body is None else f"[{float(cop_body[0]):+.3f}, {float(cop_body[1]):+.3f}]"
                        print(f"      gait detail: leftBW={lf_bw:.2f} rightBW={rf_bw:.2f} supportBW={sup_bw:.2f} tan/normal={tan_ratio:.2f} CoP_body={cop_str}")
                    controller.set_target_z(z_fall, blend_steps=40)
                elif force_start and (step % 30 == 0):
                    print(f"  [Step {step}] Waiting for valid gait before perturbation: speed={walk_diag['speed']:.3f} target={controller.walk_target_speed:.3f} lat={walk_diag['lateral_err']:+.3f} yaw={walk_diag['yaw_err_deg']:+.1f}deg GRF/BW={walk_diag['grf_bw']:.2f}")
            elif actual_step_marks['react_start'] is None:
                phase = 'perturb'
                pert_steps = step - actual_step_marks['perturb_start']
                if pert_steps >= PHASES['perturb']:
                    actual_step_marks['react_start'] = int(step)
                    phase = 'react'
                    print(f"  [Step {step}] Reaction delay (maintaining perturbation) [reaction_delay={age_params['reaction_delay']:.3f}s]")
                force_dir = np.array(fall_config['force_direction'], dtype=float)
                force_dir /= np.linalg.norm(force_dir)
                ramp_progress = np.clip(pert_steps / max(PHASES['perturb'], 1), 0.0, 1.0)
                scaled_mag = FORCE_CONFIG['magnitude']
                controller.apply_external_force(scaled_mag, force_dir, ramp_progress)
                weaken_progress = np.clip(pert_steps / max(PHASES['perturb'] + PHASES['react'], 1), 0.0, 1.0)
                controller.update_muscle_weakening(weaken_progress)
            elif actual_step_marks['fall_start'] is None:
                react_steps = step - actual_step_marks['react_start']
                phase = 'react' if react_steps < PHASES['react'] else 'fall'
                if phase == 'react':
                    force_dir = np.array(fall_config['force_direction'], dtype=float)
                    force_dir /= np.linalg.norm(force_dir)
                    scaled_mag = FORCE_CONFIG['magnitude']
                    controller.apply_external_force(scaled_mag, force_dir, 1.0)
        ...
        if phase in ('perturb', 'react', 'fall'):
            ...
            nonfoot_ground_contact = controller.has_nonfoot_ground_contact(min_normal_n=25.0)
            first_ground_body_contact = nonfoot_ground_contact or (
                pelvis_height_rest < 0.26 and
                ncon_rest >= 3
            )
            if first_ground_body_contact:
                controller.activate_impact_brake()
                if nonfoot_ground_contact or (pelvis_height_rest < 0.18 and slip_rest > 0.08):
        ...
        if phase == 'fall':
            ...
            settle_candidate = (
                pelvis_height_rest < 0.19 and
                torso_height_rest < 0.23 and
                head_height_rest < 0.38 and
                ncon_rest >= 3 and
                slip_rest < 0.28 and
                pelvis_ang_speed < 2.4 and
                fall_lean_rest > 48.0
            )
            if settle_candidate:
            ...
            if _rest_stable_counter >= 5 and not controller.rest_mode:
                print(f"  [Step {step}] Rest mode engaged (anti-slip + lie-down stabilizer)")
                controller.activate_rest_mode(z_rest if 'z_rest' in globals() else z_fall)

            if controller.rest_mode:
                controller.apply_rest_stiction()
                deep_rest = (
                    pelvis_height_rest < 0.12 and
                    torso_height_rest < 0.16 and
                    head_height_rest < 0.26 and
                    ncon_rest >= 4 and
                    slip_rest < 0.08 and
                    pelvis_ang_speed < 0.45 and
                    fall_lean_rest > 65.0
                )
                if deep_rest:
        ...
        if step % 30 == 0 or (phase in ('perturb', 'react', 'fall') and step % 5 == 0):
            ...
            if phase in ('stand', 'walk'):
                try:
                    ...
                    if step >= _xcom_log_start_step and phase == 'walk':
                    ...
                    elif phase in ('stand', 'walk'):
                        _xcom_negative_streak = 0
                    walk_fall_flag = (_xcom_negative_streak >= XCOM_NEGATIVE_CONFIRM_STEPS and phase == 'walk' and pelvis_pos[2] < 0.82)
                except Exception:
                    xcom_margin = None
                    _xcom_negative_streak = 0
            ...
            metrics = dashboard.report(
                ...
                fall_predicted=walk_fall_flag if phase in ('stand', 'walk') else (pelvis_pos[2] < 0.35),
...
print("-" * 50)
walk_metrics = [m for m in metrics_log if m.get('phase') == 'walk']
...
if walk_metrics:
    ...
    walk_mean_speed = float(np.mean([abs(m.get('pelvis_vx', 0.0)) for m in metrics_log if m.get('phase') == 'walk']))
...
pre_metrics = [m for m in metrics_log if m.get('phase') in ('stand', 'walk')]
if pre_metrics:
    ...
    walk_mean_ds = float(np.mean([1.0 if m.get('double_support', False) else 0.0 for m in walk_metrics])) if walk_metrics else 0.0
    pre_valid = (pre_h_ratio > 0.75 and walk_mean_ds < 0.55 and (walk_mean_froude if walk_metrics else 0.0) > 0.06)
    ...
    print(f"  Mean walk dbl_sup  : {walk_mean_ds:.1%}")
    age_ref = get_age_reference_band(SIM_AGE, SIM_SEX, SIM_HEIGHT)
    ds_lo, ds_hi = age_ref['double_support_band']
    speed_lo, speed_hi = age_ref['comfortable_speed_band_mps']
    speed_status = 'OK' if speed_lo <= walk_mean_speed <= speed_hi else 'MISMATCH'
    ds_status = 'OK' if ds_lo <= walk_mean_ds <= ds_hi else 'MISMATCH'
